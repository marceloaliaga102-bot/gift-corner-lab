...
export const AdminView: React.FC<AdminViewProps> = ({
  ...
  const handleResetPaymentStatus = async (orderId: string) => {
    const updated = ordersList.map(o => o.id === orderId ? { ...o, paymentStatus: 'pendiente' as const, status: 'En preparación' as const } : o);
    setOrdersList(updated);
    saveOrders(updated);
    const found = updated.find(o => o.id === orderId);
    if (found) {
  ...
  const handleManualRefreshOrders = () => {
    sfx.playClick();
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <button
                ...
                className="px-3.5 py-2 rounded-xl bg-[#272a32] hover:bg-[#32353d] border border-white/10 text-[#d2bbff] text-xs font-bold pixel-btn transition-all flex items-center gap-1.5 shadow-sm disabled:opacity-50"
                title="Actualizar lista de pedidos manualmente"
              >
                <RefreshCw className={`w-3.5 h-3.5 ${isRefreshingOrders ? 'animate-spin' : ''}`} />
                <span>{isRefreshingOrders ? 'Actualizando...' : 'Actualizar'}</span>
              </button>
                      ...
                      <div className="flex items-center justify-end gap-1.5">
                        {ord.paymentStatus !== 'aprobado' && ord.paymentStatus !== 'rechazado' && (
                          <>
                            <button
                              ...
                              className="px-2.5 py-1 rounded bg-emerald-500 hover:bg-emerald-400 text-black text-[10px] font-bold pixel-btn transition-all flex items-center gap-1 shadow-sm"
                        ...
                        {ord.paymentStatus === 'aprobado' && (
                          <div className="flex items-center gap-1.5">
                            ...
                            <button
                              type="button"
                              onClick={() => handleResetPaymentStatus(ord.id)}
                              className="px-1.5 py-0.5 rounded bg-[#272a32] hover:bg-[#32353d] text-[#ccc3d8] hover:text-white text-[9px] font-mono transition-colors border border-white/10"
